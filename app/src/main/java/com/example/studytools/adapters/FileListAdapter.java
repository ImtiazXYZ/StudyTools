...
import com.example.studytools.activity.PDFViewerActivity;
...
public class FileListAdapter extends RecyclerView.Adapter<FileListAdapter.FileListViewHolder> {
    ...
    @Override
    public void onBindViewHolder(@NonNull FileListAdapter.FileListViewHolder holder, final int position) {
        ...
        if(AppPreference.getData(mContext).getType()==2){
            holder.mDeleteButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    DataHandler.deleteFile(mContext, mFileList.get(position).getId());
                }
            });
        }
        else{
